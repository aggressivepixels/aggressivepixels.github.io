...
main = do
  ...
  posts <-
    Array.sort <$> for postsFiles \postFile -> do
  ...
  for_ posts \(Tuple post rawMarkdown) ->
    case parseMd rawMarkdown of
      Left err ->
        throw $ "when parsing the content of \"" <>
          Post.getTitle post <> "\": " <> err
  ...
  where
    doWipe existingFile = do
      ...
      if Stats.isDirectory stats
        then doWipeDir existingFile
        else FS.unlink existingFile
    doWipeDir dir = do
